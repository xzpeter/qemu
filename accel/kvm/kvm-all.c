...
/**
 * kvm_physical_log_clear - Explicitly clear the kernel's dirty bitmap
 *
 * NOTE: this will be a no-op if we haven't enabled manual dirty log
 * protection in the host kernel because in that case this procedure
 * is already done by the log_sync() hook.
 *
 * @kml:     the kvm memory listener
 * @section: the memory range to clear dirty bitmap
 * @bitmap:  the bitmap to use to clear the range.  If %NULL, then we
 *           clear all the pages covered by the range.
 */
                                  ...
                                  const unsigned long *bitmap)
{
    ...
    bmap_start /= qemu_real_host_page_size;
    ...
    bmap_npages = DIV_ROUND_UP(size + start_delta, KVM_CLEAR_LOG_ALIGN)
        << KVM_CLEAR_LOG_SHIFT;
    end = (mem->start_addr + mem->memory_size) / qemu_real_host_page_size;
    if (bmap_npages > end - bmap_start) {
        bmap_npages = end - bmap_start;
    }
    start_delta /= qemu_real_host_page_size;

    if (!bitmap) {
        bmap_clear = bitmap_new(bmap_npages);
        bitmap_fill(bmap_clear, bmap_npages);
        /* Clear out the extended start region */
        if (start_delta) {
            bitmap_clear(bmap_clear, 0, start_delta);
        }
        d.dirty_bitmap = (void *)bmap_clear;
    } else {
        if (start_delta) {
            /*
             * Slow path: We've filled in empty spaces before the real
             * dirty bitmap, we need to shift the origin bitmap before
             * passing it to the kernel
             */
            bmap_clear = bitmap_new(bmap_npages);
            bitmap_copy_with_dst_offset(bmap_clear, bitmap, start_delta,
                                        bmap_npages - start_delta);
            /*
             * Make sure the start margin region is with zeros
             * otherwise we might erase dirty bits that we don't
             * really know of, which can cause misterious data
             * corruption.
             */
            bitmap_clear(bmap_clear, 0, start_delta);
                          ...
                          const unsigned long *bitmap)
{
    ...
    r = kvm_physical_log_clear(kml, section, bitmap);
    if (r < 0) {
...
static int kvm_init(MachineState *ms)
{
    ...
    s->manual_dirty_log_protect =
        kvm_check_extension(s, KVM_CAP_MANUAL_DIRTY_LOG_PROTECT);
    if (s->manual_dirty_log_protect) {
        ret = kvm_vm_enable_cap(s, KVM_CAP_MANUAL_DIRTY_LOG_PROTECT, 0, 1);
        if (ret) {
